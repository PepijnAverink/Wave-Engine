...
{
public:
	Perspective(const glm::vec3& pos, const glm::vec3& lookAt, float fov, float _near, float _far, float aspect,
		float speed = 20.0f, float sensitivity = 0.1f, float yaw = 0.0f) :
		Camera(pos, lookAt, speed, sensitivity, yaw), m_FOV(fov), m_Aspect(aspect), m_Near(_near), m_Far(_far), m_IsDirty(false)
	{
